...
# 실수 기반 이분탐색 => 고정횟수반복(필요한 정밀도에 도달할 반복 횟수를 미리 계산하여 최소 그 이상 반복)
#   초기 구간 폭(left - right) = 10^9
#   반복마다 구간은 절반으로 줄어듦. 이 과정을 k번을 반복하게 된다고 한다면,
#   구간 폭은 10^9 / 2^k
#   소수점 4자리까지 요구하니까 정밀도는 10^-4
#   구간 폭이 이보다 작아지면 left와 right는 사실상 같은 값
#   10^9 / 2^k <= 10^-4
#       10^13 <= 2^k -> log_2(10^13) <= k -> 대충 k는 최소 44번 이상만 수행하면 됨.
#   일반화 = k >= log_2(초기 구간 폭 / 목표 정밀도)
