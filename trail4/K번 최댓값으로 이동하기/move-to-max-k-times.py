...
# 기억해야 할 것 
# 1. 센티널로 상태 표현 시, 그 상태를 확인할 때에도 반드시 센티널 자체와 비교해야 함.
#   다른 변수로 우회 추측하면 오염된 값이 다음 반복으로 전파되는 버그가 생김
# 2. 튜플 우선 순위 비교를 잘 기억해야 함.
#   (값, -행, -열)처럼 부호를 조정한 튜플을 만들면, Python의 사전순 비교로 여러 단계 타이브레이크를 한 번에 처리 가능
# 3. 사전순 비교
#   문자열 자체를 대소비교로 사전순 비교 가능함.